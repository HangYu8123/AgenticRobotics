...
> **Drift note (2026-07-19 15:28, Function Update ID 7):** Two new candidate skills — `skills/iterated/reinforcing-with-rl/` (RL fine-tune against an env reward to escape the demonstration distribution — new-behavior discovery) and `skills/iterated/harvesting-rollouts/` (autonomous self-generated rollout data + reward logging; record + label when no env reward) — bringing the library to **24 skills** (7 validated / 16 candidate / 1 deprecated). `robot_agentic_training_flow.md` gained: a held-out **[acceptance gate]** made *part of* the **[exit condition]** (opt-in via the objective's `acceptance_eval`/`acceptance`, resolved only at Step 4 with a new `exit_status: ACCEPTANCE_FAILED` outcome — this **enforces** the previously spec-only acceptance design flagged in Function Update ID 6; reuses `agentic_robot/scoring.py::acceptance_passed`); a **[measurement integrity]** gate (Step 0 pins repo/run-local eval-script sha256; Step 3f verifies it + scans workers for eval/harness tampering → verdict `ERROR`); and a **Backend independence** framing (LeRobot/SmolVLA/LIBERO are the reference/validation backend, not the only target — the contract is a resumable trainer + an `eval_command` writing `eval_info.json`). No Python/recovery/CLI changes.

> **Drift note (2026-07-19 16:31, Function Update ID 8):** Skills-library-only change (no flow, Python, or CLI edits). Library is now **26 skills** (7 validated / 18 candidate / 1 deprecated). Two new: `skills/iterated/conditioning-on-advantage/` (RECAP-style advantage-conditioned RL — the lever for **flow-matching** policies like SmolVLA/pi0, which expose no action log-probs for PPO/GRPO) and `skills/iterated/synthesizing-skills-from-rollouts/` (mines ≥5 completed ledger rounds for cross-round procedures/traps that per-round Step 3(h) registration cannot see; returns **proposals only** — the controller still applies them under `LIBRARY.lock`). Backend-coupled skills were **restructured, not deleted**, into a two-layer body — a backend-independent `## Do` contract plus a labeled `### Reference backend (LeRobot / SmolVLA / LIBERO)` subsection preserving every verbatim command and trap (`continuing-training`, `curating-dataset`, `tuning-action-horizon`, `measuring-the-gate`). `reinforcing-with-rl` gained a cited RL-recipe table keyed on **policy class** (token-VLA recipes such as SimpleVLA-RL and RIPT-VLA do **not** run on a flow-matching action expert); `researching-online` gained a **sweep mode** (proactive newest-methods search that emits candidate-skill proposals, not only stagnation-triggered diagnosis). `skills/index.md` gained a *Backend portability* note, RATs-derived numeric lifecycle thresholds (≥3 rounds/≥50% → validated; ≥10/≤20% → deprecated) with an explicit precedence rule that `evidence_tier` wins for unattended selection, and a ~5-round curation cadence.

> **Drift note (2026-07-19 17:11, Function Update ID 9):** New **`context/RoboticKnowledges/`** — a robotics reference library of 11 Markdown files (~1,505 lines): `knowledge_index.md` plus one `README.md` in each of ten category subfolders (`ros`, `hardware`, `reinforcement-learning`, `imitation-learning`, `diffusion-and-flow-matching`, `vla`, `classic-control-and-planning`, `kernel-methods`, `mechanical-engineering`, `agentic-ai`). **Important scoping fact:** unlike its siblings under `context/`, this tree is **not** a vendored mirror of a pack file and is **never read by `robot_agentic_training_flow.md` at runtime** — it is background reading for operators and research subagents, so changes here cannot affect a run. Where it overlaps the skills library the **skill wins**; `reinforcement-learning/` and `vla/` explicitly cross-reference `skills/iterated/reinforcing-with-rl/SKILL.md` as the source of truth for VLA-RL recipes rather than forking its paper table. Content was compiled under a live-web-only anti-fabrication contract (every entry cites a fetched URL; `award:`/`lab:` claims only where a fetched page confirms them; explicit empty year-slots preferred over force-fitted papers). No Python, flow, skills, or CLI changes.

> **Drift note (2026-07-19 17:24, Function Update ID 10):** `robot_agentic_training_flow.md` gained a tenth round-loop sub-step, **Step 3(j) Record**, which appends one advisory row per committed round to the repo-root `LoopRefineRewards/<run_id>.md` — round status, raw metric → target, a ≤4-word gist of the controller's own behavior and of each of the three Step 3(c) advisory reports, a Yes/No "did I use it" flag per report, and whether the round beat the previous round's raw metric. Supporting terms: a new `[refine signal]` `[outputs]` row, a `refine_signal` sub-object inside the durable `[round log inputs]` (persisted at Step 3(d), the only moment all three reports coexist — Step 3(i) discards worker output as its context-rot defense), and a `refine_signal_recorded` commit-keyed durable field. **Ordering is load-bearing:** (j) must run after `ledger_committed` and *before* (i)'s round-advance replacement, because `round_log_inputs`/`score_snapshot` are single fields the next round overwrites; recovery step 6 completes a missed (j) before advancing but never holds the advance for it. The record is advisory and offline — never read back into loop state, no control decision depends on it — so it does not weaken Invariant 6; its consumer is the offline revision of the flow program. This makes **two** in-repo growing artifacts (`skills/` and `LoopRefineRewards/`). No Python, recovery, CLI, or skills-library changes (`recovery/round_state.schema.json` needed none — it is `additionalProperties: true` at top level).

> **Drift note (2026-07-20 12:36, Function Update ID 11):** `robot_agentic_training_flow.md` (+ `objective.example.yaml`) — **promotion/scoring hardening** (spec + template only; no Python/recovery/CLI/skills-library change). Step 3(g) now promotes **outright only past a 1.5× margin** over the [noise band] (or ≥ 1.5 × [min_delta]); any *smaller* directionally-improving delta is routed into the pre-existing confirmation protocol (pooled re-test, or the gating paired McNemar/bootstrap test at α = 0.05), so a single band-edge noise spike can no longer become a permanent champion (the biggest of the four fixes). Mirrored in the [improvement rule] gate (with a note that a per-round margin bounds one comparison's false-promotion rate, *not* the cumulative adaptive-reuse selection bias — the [acceptance gate] is that guard), the `NO PROGRESS` verdict, [best observed], the [min_delta] definition, and the `objective.example.yaml` min_delta comment (set it **above** the measured same-checkpoint/test-retest spread, not equal to it). Step 3(c) now **forbids skipping the analysts whenever the previous round promoted a new [best checkpoint]** (`PROGRESS`/`EXIT_REVALIDATED`, not the round-0 baseline) — re-diagnose the new champion, never reuse a pre-promotion diagnosis. Step 3(d) adds a **data-before-recipe** root-cause clause. Step 3(g)(ii) adds an **advisory eval-staleness guard** (the champion's stored per-episode outcomes are trusted for the paired test only while the eval is unchanged since it was measured; on a possible [measurement integrity] re-pin, re-measure the champion fresh or skip the paired path with a distinct note), and [acceptance_eval] now **recommends enabling the locked holdout for long campaigns**. **Rejected as over-engineered:** a durable per-attempt eval-pin-provenance field, per-round champion re-eval, cryptographic eval attestation, a campaign-state tracker, and alpha-spending machinery.
...
│   └── RoboticKnowledges/            # NOT a pack mirror and NOT read at runtime —
│                                     # a dated robotics reference library (10 categories,
│                                     # knowledge_index.md + one README.md per subfolder)
