...
> **Drift note (2026-07-20 12:36, Function Update ID 11):** `robot_agentic_training_flow.md` + `objective.example.yaml` only — **promotion/scoring hardening**, no Python/recovery/CLI/skills-library change. Step 3(g) promotes outright only past a **1.5× margin** over the [noise band]/[min_delta]; smaller improving deltas go through the existing confirmation/paired-test protocol (α = 0.05) — killing the "single noise spike → permanent champion" failure. Step 3(c) forbids skipping analysts after a promotion (re-diagnose the new champion; no stale-diagnosis reuse); Step 3(d) adds a data-before-recipe clause; Step 3(g)(ii) adds an advisory champion-eval-staleness guard; [acceptance_eval] recommends the locked holdout for long campaigns. `objective.example.yaml`'s min_delta comment now says to set it above the measured same-checkpoint spread. Reviews: simplify:local (1 punctuation trim), code_review:local (0 Critical/Required; 3 low-severity applied, 2 FYI). No test surface touched.
